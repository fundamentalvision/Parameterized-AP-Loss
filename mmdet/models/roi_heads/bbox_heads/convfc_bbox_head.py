...
from mmdet.core import (force_fp32, vectorize_labels, giou, iou)
...
@HEADS.register_module()
class PAPLossShared2FCBBoxHead(ConvFCBBoxHead):

    def __init__(self, fc_out_channels=1024, ctrl_points=[0.0], reg_weight=1.0, num_topk=1e5, reg_input='giou', loss_form='searched', *args, **kwargs):
        super(PAPLossShared2FCBBoxHead, self).__init__(
            num_shared_convs=0,
            num_shared_fcs=2,
            num_cls_convs=0,
            num_cls_fcs=0,
    ...
    def forward(self, x):
        ...
        cls_score = self.fc_cls(x_cls) if self.with_cls else None
        bbox_pred = self.fc_reg(x_reg) if self.with_reg else None
        cls_score, bbox_pred = GradScale.apply(cls_score, bbox_pred, self.reg_weight)
        return cls_score, bbox_pred
    ...
    def loss(self,
        ...
        if bbox_pred is not None:
            ...
            if pos_inds.any():
                if self.reg_decoded_bbox:
                    bbox_pred = self.bbox_coder.decode(rois[:, 1:], bbox_pred)  # this is true, so box in [x1, y1, x2, y2]
                if self.reg_class_agnostic:
                    bbox_pred = bbox_pred.view(bbox_pred.size(0), 4)
                    pos_bbox_pred = bbox_pred[pos_inds.type(torch.bool)]
                else:
                    # will go this way
                    bbox_pred = bbox_pred.view(bbox_pred.size(0), -1, 4)
                    pos_bbox_pred = bbox_pred[pos_inds.type(torch.bool), labels[pos_inds.type(torch.bool)]]
            
                num_b, num_cls = cls_score.shape
                # get one_hot labels
                one_hot_labels = torch.zeros([num_b, num_cls]).to(labels)
                one_hot_labels[pos_inds.type(torch.bool), labels[pos_inds.type(torch.bool)]] = 1
                ...
                if self.reg_input == 'giou':
                    pos_ious = giou(pos_bbox_pred, bbox_targets[pos_inds.type(torch.bool)])
                    pos_ious = (pos_ious + 1)/2
                # iou
                if self.reg_input == 'iou':
                    pos_ious = iou(pos_bbox_pred, bbox_targets[pos_inds.type(torch.bool)])
                # l1
                if self.reg_input == 'l1':
                    pos_ious = 1 - torch.abs(pos_bbox_pred - bbox_targets[pos_inds.type(torch.bool)]).sum(-1)
                    pos_ious = torch.clamp(pos_ious, min=0, max=1)
                
                ious = torch.zeros([num_b]).to(pos_ious)
                ious[pos_inds.type(torch.bool)] = pos_ious
                ious = ious.view(-1, 1).repeat(1, num_cls)
                ious = ious * one_hot_labels
                
                loss = paploss(cls_score, ious, one_hot_labels, self.ctrl_points, self.num_topk, self.loss_form)
                losses['loss_pap'] = loss

            else:
                losses['loss_pap'] = cls_score.sum() * 0 + bbox_pred.sum() * 0
