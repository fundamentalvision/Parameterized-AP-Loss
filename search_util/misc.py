...
class WarmUpScheduler:
    def __init__(self, optimizer, cfg):
        ...
        self.warmup_iters = cfg.lr_config.warmup_iters
        self.warmup_ratio = cfg.lr_config.warmup_ratio
        self.steps = cfg.lr_config.step
        
        for group in self.optimizer.param_groups:
            group.setdefault('initial_lr', group['lr'])
            self.base_lr = [group['initial_lr'] for group in self.optimizer.param_groups]
    
    def step(self, iters, epochs):
        if iters < self.warmup_iters:
            k = (1 - iters / self.warmup_iters) * (1 - self.warmup_ratio)
            for group, lr in zip(self.optimizer.param_groups, self.base_lr):
                group['lr'] = lr * (1 - k)
        elif epochs in self.steps:
            k = len(self.steps)
